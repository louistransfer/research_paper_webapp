...
def Equity(
    ...
    if public:
        org_.append("public")
    if len(org_) > 0:
        value_.append(" or ".join(org_))

    if active:
        status_.append("active")
    if inactive:
        status_.append("inactive")
    if len(status_) > 0:
        value_.append(" or ".join(status_))

    if countryprimaryquote:
        value_.append("countryprimaryquote")
...
def GMEDIAN(groupby_field, value_field):
    ...
    """
    if (
        (groupby_field is not None)
        and (len(groupby_field) > 0)
        and (value_field is not None)
        and (len(value_field) > 0)
    ):
...
def WAVG(weight_field, field):
    ...
    """
    if (
        (weight_field is not None)
        and len(weight_field) > 0
...
def TEXTAV(field, custom_string):
    ...
    if (
        (field is not None)
        and (len(field) > 0)
        and (custom_string is not None)
        and (len(custom_string) > 0)
    ):
        condition = []
...
def IF(condition, true_field, false_field):
    ...
    if (
        ...
        and (len(condition) > 0)
        and (true_field is not None)
        and (len(true_field) > 0)
        and (false_field is not None)
        and (len(false_field) > 0)
    ):
...
def POW(field, power=1):
    ...
    """
    if (field is not None) and (len(field) > 0):
        values_ = "POW({},{})".format(field, power)
    else:
        values_ = ""
    return values_
...
def ZSCORE(field, period="1M"):
    ...
    """
    if field != "" and period != "":
        value_ = "({}-AVG({}(edate=-{})))/STD({}(edate=-{}))".format(
            field, field, period, field, period
        )
    else:
        value_ = ""
    return value_
